...
* __Subset the **Core genome**:__ Filter by genes that are found in both genotypes (`No..isolates` column == 1)
...
### Exercise
 ...
 > Find a gene or two that you think might be relevant to the virulence.
...
A very widely used web-based tool for microbial genome annotation is **RAST**. Your task is to figure out how to use this program (which is similar to Prokka), use it to re-annotate the `E.coli_TY.2842`, and then use `roary` again to identify *Core* and genome-specific genes for the two bacteria. Present a Venn diagramm of the results of the RAST annotation.
